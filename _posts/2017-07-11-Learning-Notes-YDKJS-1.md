...
* eg：var a = 2；可以被编译器分割为var,a,=,2,; 空格是否会被当作词法单元，取决于空格在 这门语言中是否具有意义。
2. 解析/语法分析  
...
### 理解作用域 
1. 介绍以下三个关键的概念
* 编译器: 用来在引擎执行代码前提供给引擎代码 PS：编译器就是一个给作用域介绍人的人,作用域的变量都是通过他来进入的
* 引擎：用来负责执行和编译的环境 PS：从作用域里面获取环境
* 作用域：负责收集并维护由所有声明的标识符(变量)组成的一系列查询，并实施一套非常严格的规则，确定当前执行的代码对这些标识符的访问权限。PS：将编译器介绍的人全部存储起来
* LHS和RHS：“赋值操作的目标是谁(LHS)”以及“谁是赋值操作的源头(RHS)”。PS：理解为引擎查询作用域的动作
...
* with函数:简单来说with函数回味{}以内的语句在当前的位置以内创建了一个作用域而且自动放入了吧obj对象当中的属性放了进去.
``` js
//with的用法
var obj = {  
a: 1,
...
* eval和with我认为并不是作为词法作用域的范围,以为词法作用域是在编译前就做好的,所以这个叫做欺骗词法作用域,当然因为是动态的所以会消耗性能
...
* 下面说的方法其实是通过一些其他的表达式干扰编译器的判断,让编译器认为这并不是一个声明
* 对于函数的表达式和函数的声明还有立即执行函数可以看看这两个博主的文章[看看我 ](https://my.oschina.net/u/2331760/blog/468672)[还有我](https://my.oschina.net/u/2331760/blog/468672)
...
```
* `1好奇地是并不知道为什么(var a)就不能被解析为一个非声明的语句,我认为的可能性是因为js当中会有 let a = function(){}的代码也就是说function(){}本身斌不会全部当做一个生命语句他出生的时候就觉定了会根据上下文来确定他是什么语句所以你肯定没见过 let a = var c 这样的语句格式,所以js当中你只能通过代码改变编译器对function的认知
...
baz(); // 2 —— 朋友，这就是闭包的效果。
```
* 看起来你并不觉得这有什么牛逼的地方,但是其实js当中闭包是十分常用的功能
```js
